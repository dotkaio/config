...
export FPATH=$CONFIG/completions:$FPATH
# export FPATH=/usr/local/share/zsh-completions:$FPATH

source $CONFIG/terminal/autosuggestion/init
source $CONFIG/terminal/highlight/init
# if get local insecure
# rm -f ~/.zcompdump; compinit
# chmod go-w '/usr/local/share'
# chmod -R go-w '/usr/local/share/zsh'
#

# if vscode is installed, move the code command to the usr/local/bin folder

# PROMPT
prompt='%F{cyan}%h %F{green}%B%/%F{red}%b $(branch_name)%f
>_ '

# prompt='%F{red}[%F{cyan}%D{%a %y/%m/%d %R %Z}%F{red}]%F{red}[%F{green}%l%F{red}]%F{red}[%F{cyan}x86_64/darwin21.0/21.6.0%F{red}]%F{red}[%F{cyan}5.8.1%F{red}]
# %F{red}<%F{green}%n@%m%F{white}:%F{yellow}%~%F{red}>
# %fzsh%(2L./5.) %B%h%b %(?..[%?%1v] )%(2v.%U%2v%u.)%f%B%#%b'
